...
sb_gfdl_clims <- sb_gfdl |>
  ...
  filter(depth <= 500) |>
  drop_na(temperature) |>
  #matching depth bins into the four depth layers of the southern Benguela Atlantis model
...
Now we have a grid of our model, which we can use to rasterise the shapefile of Southern Benguela. We will use the `number` column as the identifier for each grid cell.    
...
sb_temp_corr_box <- sb_gfdl_box |> 
  ...
  left_join(sb_gfdl_clim_box, by = c("layer", "month", "number")) |> 
  left_join(sb_w_clim_box, by = c("layer", "month", "number")) |> 
  #Apply correction
  mutate(corrected_temp = (w_temp_ts-w_temp_gfdl)+w_temp_woa)

#plot the temperature time series ranging from January 1961 to December 2010
sb_temp_corr_box |> 
filter(number== 2 & layer ==1) |>
ggplot(aes(date)) +
...
If the first row has a difference equal or larger than +/-1 in any layer or polygon, then we need to use the bias corrected temperature (`corrected_temp`).
