...
Now we have a grid of our model, which we can use to rasterise the shapefile of Southern Benguela. We will use the `BOX_ID` column as the identifier for each grid cell.    
  
```{r}
#Create raster mask
cb_mask <- rasterize(benguela, ras, field = "BOX_ID", fun = max, 
                     background = NA) |> 
...
#Check results
cb_mask |> 
  ggplot(aes(lon, lat, fill = BOX_ID))+
  geom_raster()
```
